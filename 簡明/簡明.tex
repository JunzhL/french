...
\chapter{Leçon 3}
    \section{E4}
        \begin{enumerate}
            ...
            \item C'est Sabine. Oui, elle habite à Grenoble.
            \item Oui, elle est journaliste. Elle habite à Nice.
        \end{enumerate}

    \section{E6}
        \begin{enumerate}
            \item Thomas est facteur. Il habite à Paris.
            \item Victor est cinéaste. Elle habite à Vice.
...
\chapter{Leçon 4}
    ...
    \section{E4}
        \begin{enumerate}
            ...
            \item Est-ce que Li Hong est votre chef de classe?
        \end{enumerate}
        Correction:
        \begin{enumerate}
            \item Où habite Philippe?
        \end{enumerate}
...
\chapter{Leçon 6}   
    ...
    \section{E6}
        \begin{itemize}
            ...
            \item Ce sont les manteaux de Marie et de Monique.
        \end{itemize}

    \section{E7}
        \begin{enumerate}
            \item Qu'est-ce que c'est?
            \item Ce sont des bancs.
            \item Est-ce que ce sont des magnétophones?
            \item Non, ce ne sont pas des magnétophones, ce sont des radios.
            \item Est-ce que ce sont les cassettes de Jacques?
            \item Non, ce n'est pas les cassettes de Jacques. Ce sont les cassettes de Pascal.
        \end{enumerate}

\chapter{Leçon 7}
    \section{Volcab}
        \begin{itemize}
            \item le camion (truck)
        \end{itemize}
    
    \section{Grammar}
        
        \begin{itemize}
            \item Capitalize for name, place name and nationality.
            \item Article is ignored when expressing occupation, identity, for example:
            \begin{itemize}
                \item Je suis étudiant.
                \item Il est ingénieur.
            \end{itemize}
            \item Transition from mas. to fem.
            \begin{itemize}
                \item -ien, add ne
                \item -eur, change ``eur'' to ``rice'', sometimes to ``euse''
                \item some only has masculine: professeur, médecin, pilote, ingénieur. One way is to add ``femme'': une femme professeur.
            \end{itemize}
        \end{itemize}

\chapter{Leçon 8 - Review}
    \section{Volcab}
        \begin{itemize}
            \item causer avec quelqu'un (to chat with someone)
        \end{itemize}

\chapter{Leçon 9}
    \section{Volcab}
        \begin{itemize}
            \item être en \dots année
            \item être dans le département de
        \end{itemize}
    When expressing property, don't need le/la/les, however needed when expressing relation.

\chapter{Leçon 10}
    \section{Grammar}

        \begin{itemize}
            \item for weekdays, use ``le'' to express repeating every week.
            \item profiter de qch pour 
        \end{itemize}

\chapter{Leçon 11}
    \begin{itemize}
        \item être + à (something belongs to someone)
        \item être de + place (comes from that place)
        \item qch + être difficile + à + inf (hard to do)
        \item aider qn dans (help someone in something)
        \item aider qn à + inf (help someone to do something)
    \end{itemize}

\chapter{Leçon 12}
    \begin{itemize}
        \item ancien (if before noun, means former, if after noun, means old)
        \item prendre (can be used as buy)
        \item une dizaine d'années (about ten or more years), this pattern (de + noun) can be applied to other numbers.
        \item être à la retraite / être en retraite / prendre sa retraite
        \item je vous en prie (no problem)
    \end{itemize}

    \section{E2}
        Ex: Est-ce que tu travailles? Non, je ne peux pas travailler.
        \begin{enumerate}
            \item Non, je ne peux pas aider mes camarades.
            \item Non, nous ne pouvons pas faire ces exercices.
            \item Non, Pierre ne peut pas venir.
            \item Non, elle ne peut pas apprendre le japonais.
            \item Non, je ne peux pas suivre la concierge.
            \item Non, il ne peut pas suivre ses amis.
        \end{enumerate}
    
    \section{E3}
        \begin{enumerate}
            \item Non, je ne veux pas aider mes camardes.
            \item Non, nous ne voulons pas faire ces exercices.
            \item Non, Pierre ne veut pas venir.
            \item Non, elle ne veut pas apprendre le japonais.
            \item Non, je ne veux pas suivre la concierge.
            \item Non, il ne veut pas suivre ses amis.
        \end{enumerate}

    \section{E4}
        \begin{enumerate}
            \item to describe the purpose of a noun, use noun + à
            \item être de combien (use to ask for price)
            \item feminine and non consonant country name, use en or de
        \end{enumerate}

\chapter{Leçon 9-13 Révision}

    \section{E4}
        \begin{enumerate}
            \item Je n'aime pas ces tableaux.
            \item Ce n'est pas un magnétophone.
            \item Je n'ai pas une moto.
            \item Ils n'ont pas de cours aujourd'hui.
            \item Paul n'a pas deux frères.
            \item Pierre ne téléphone pas à Marie.
            \item Ils ne font pas ces exercices ce soir.
            \item M. Duval n'habite pas au troixième étage.
        \end{enumerate}
        Correction:
        \begin{enumerate}
            \item Je n'ai pas de moto.
        \end{enumerate}

    \section{E8}
        \begin{enumerate}
            \item Qu'est-ce?
            \item Est-ce que Pascal travaille dans cette agence?
            \item Quel âge a votre frère?
            \item À quelle étage habite Monique?
            \item Voulez-vous lire ces romans?
            \item Est-ce que ces exercices difficiles?
        \end{enumerate}
        Correction:
        \begin{enumerate}
            \item Quel âge a-t-il, votre frère?
            \item Est-ce que ces exercices sont difficiles?
        \end{enumerate}

    \section{E9}
        \begin{enumerate}
            \item Il travaille en France.
            \item Marie vient du Canada.
            \item Je suis dans le département de français.
            \item Ils sont en troisième année.
            \item Ce sont des ouvriers. Il travaillent dans une usine. Cette usine est grande.
            \item C'est la robe de Nathalie.
            \item Ce sont des cassettes de classe A.
            \item Il est ingénieur de Renault.
        \end{enumerate}
        Correction:
        \begin{enumerate}
            \item Ce sont des ouvriers. Il travaillent dans une usine. L'usine est grande. (cette also works)
            \item Ce sont les cassettes de la classe A. (the previous has a different meaning)
            \item Il est ingénieur chez Renault. (chez to express works at)
        \end{enumerate}

    \section{E10}
        \begin{enumerate}
            \item Ce sont les livres de Paul.
            \item À qui sont ces vélos?
            \item Il y a beaucoup d'etrangers dans leur classe.
            \item Elle est à Paris depuis trois jours.
            \item Je ne prends pas l'ascenseur, j'aime prendre l'escalier.
            \item Vous avez beaucoup de journaux et de revues français?
            \item Le dimanche, ils viennent souvent voir leurs amis.
            \item Mon bureau est près d'une banque.
            \item Voulez-vous louer un vélo?
            \item M.Martin est en retraite. Il voyage maintenant aux États-Unis.
        \end{enumerate}
        Correction:
        \begin{enumerate}
            \item Ces livres sont à Paul. (être à + person, previous also works)
        \end{enumerate}

\chapter{Leçon 14}
    \section{Notes}
        \begin{itemize}
            \item marcher (also means operate): Cet ordinateur ne marche pas très bien.
            \item demander qch à qn
            \item use definitive article when expressing occupation after M/Mad: Monsieur le président.
            \item retarder de + time (to express slow), similarily avancer de + time
        \end{itemize}
    
    \section{L'impératif}
        Only tu (drop -s), nous, vous. sois, soyons, soyez, aie, ayons, ayez.
    
    \section{Time}
        Il est \dots :
        \begin{itemize}
            \item quatre heures et quart
            \item sept heures trois quarts 
            \item six heures moins le quart
            \item six heures moins cinq
            \item du matin / de l'après midi / du soir.
        \end{itemize}
        for difference, article ``le'' needed. 
        \\
        At a time, use à:
        \begin{itemize}
            \item Il vient à six heures.
            \item Chaque jour il se couche à minuit.
        \end{itemize}

\chapter{Leçon 15}
    \section{E3}
        \begin{enumerate}
            \item Aujourd'hui, j'ai beaucoup de devoirs à faire. Je ne peux pas aller au cinéma avec toi ce soir.
            \item Quand vas-tu voir ton camarade de classe?
            \item Les enfants de chinois entrent à l'école depuis six ans.
            \item Nous avons des corus tous les jours, sauf le samedi après-midi et le dimanche.
            \item Cette fille est parasseuse, elle n'aide pas ses parents à faire le ménage.
            \item Nous passons les examens début juillet. Quand les passez-vous?
            \item Comment va ton frère? Je ne le vois pas beaucoup.
            \item Il va bien, merci. Mais en ce moment, il n'est pas à Beijing.
        \end{enumerate}
        Correction:
        \begin{enumerate}
            \item Les enfants chinois entrent à l'école à l'âge de six ans.
        \end{enumerate}

\chapter{Leçon 16}
    \section{Notes}
        \begin{itemize}
            \item être obligé de + inf (have to)
            \item prier qn de + inf (please someone to do something)
            \item vacances (usually used as plural)
        \end{itemize}

\chapter{Leçon 17}
    \section{Notes}
        \begin{itemize}
            \item tout à fait (completely)
            \item un(e) de (one of \dots)
            \item présenter X à Y
            \item venir de + inf (just finished something)
            \item être au chômage (unemployed)
            \item faire la queue (line up)
            \item trouver une place (une place can be used to express occupation, job)
            \item faire la connaissance de qn (to meet someone)
        \end{itemize}

\chapter{Leçon 18}
    \section{Conjugation}
        \begin{itemize}
            \item neiger: il neige
            \item pleuvoir: il pleut
            \item falloir: il faut
        \end{itemize}
    
    \section{Note}
        \begin{itemize}
            \item à \dots de + location (to express distance): à cent mètres d'ici.
            \item tomber en panne (broken down)
            \item être en panne (to express state)
            \item use ``au'' for ``printemps'', for actual season, use ``en''
            \item for month, use ``en'' or ``au mois de + month''
            \item for date, use ``le''
        \end{itemize}
    
    \section{E2}
        \begin{enumerate}
            \item De quelle région êtes-vous? Je suis à Toronto, Canada.
            \item Est-ce qu'il fait froid en hiver? Oui, il fait froid en hiver ici.
            \item Est-ce qu'il neige quelquefois? Il neige toujours en hiver.
            \item Est-ce qu'il pleut souvent en éte? Non, il ne pleut pas souvent en été.
            \item Savez-vous patiner? Non, je ne sais pas patiner.
            \item Savez-vous nager? Oui, je sais nager.
            \item Aimez-vous faire des excursion? Oui, j'adore faire des excursions.
            \item Quelle saison aimez-vous? J'aime l'hiver.
        \end{enumerate}
        Correction:
        \begin{enumerate}
            \item De quelle région êtes-vous ? Je suis de la région de Toronto, au Canada.
            \item Est-ce qu'il fait froid en hiver ? Oui, il fait froid ici en hiver.
            \item Est-ce qu'il neige quelquefois ? Oui, il neige souvent en hiver.
        \end{enumerate}

\chapter{Leçon 19}
    \section{Note}
        \begin{itemize}
            \item travailler comme + occupation: no article after comme, the noun following agrees in number and gender.
            \item venir + inf (come to do something)
            \item ne plus, ne pas, ne jamais: when used for inf in ``no'' sentence, usually in front of the inf. (je suis déçu de ne pas le voir.)
            \item parler + language (omits article)
            \item A useful rule:
                \begin{itemize}
                    \item parler + language \(\to\) no article
                    \item être + nationality adjective \(\to\) no article
                    \item country alone \(\to\) usually has article
                    \item en / au / aux + country \(\to\) special country form
                    \item language or nationality as a noun \(\to\) use article
                \end{itemize}
        \end{itemize}

    \section{E2}
        \begin{itemize}
            \item Est-ce que vous voyagez souvent? Oui Je voyage souvent.
            \item Aimez-vous voyager seul ou en groupe? J'aime voyager en groupe de trois ou quatre personnes.
            \item Quand vous partez en voyage, vous prenez le train ou l'avion? Je prend l'avion.
            \item Est-ce que vous prenez des photos pendant le voyage? Oui, je prend des photos souvent pendant le voyage.
            \item Quand il faut avoir un passeport? J'obtiens mon passeport en 2010.
        \end{itemize}
        Correction:
        \begin{itemize}
            \item Quand vous partez en voyage, vous prenez le train ou l'avion ? Je prends l'avion.
            \item Est-ce que vous prenez des photos pendant le voyage ? Oui, je prends souvent des photos pendant le voyage.
            \item Quand faut-il avoir un passeport ? Il faut avoir un passeport quand on voyage à l'étranger.
        \end{itemize}

    \section{E10}
        \begin{itemize}
            \item Oui, montrez votre passeport, s'il vous plaît.
            \item Je vais passer des vacances à Nice, chez mon oncle.
            \item Aujourd'hui, Marie n'a pas de cours. Elle veut profiter ses temps libre pour voir ses amis, Pierre et Paul.
            \item Les étudiants vont souvent à la campagne et travaillent comme les paysans dans champs.
        \end{itemize}
        Correction:
        \begin{itemize}
            \item Oui, montrez-moi votre passeport, s'il vous plaît.
            \item Je vais passer mes vacances à Nice, chez mon oncle.
            \item Aujourd'hui, Marie n'a pas de cours. Elle veut profiter du temps libre pour voir ses amis, Pierre et Paul.
            \item Les étudiants vont souvent à la campagne et travaillent avec les paysans aux champs.
        \end{itemize}
...
\chapter{Leçon 20}  
    \section{Note}
        \begin{itemize}
            \item Cela fait 
            \item être de + amount
            \item faire partie de (belongs to ...)
        \end{itemize}
    \section{E2}
        \begin{enumerate}
            \item Habitez-vous dans un immeuble? Oui, j'habite dans un immeuble.
            \item À quel étage habitez-vous? J'habite à cinqième étage.
            \item Combien de pièces y a-t-il votre appartement? Il en y l'a trois.
            \item Est-ce qu'il y a la cuisine et le W.C.? Oui, il y l'a.
            \item Y a-t-il aussi la salle de bains? Oui, il y l'a.
            \item Est-ce que le loyer est élevé? Oui, le loyer est très cher.
        \end{enumerate}
        Correction:
        \begin{enumerate}
            \item À quel étage habitez-vous ? J'habite au cinquième étage.
            \item Combien de pièces y a-t-il dans votre appartement ? Il y en a trois.
            \item Est-ce qu'il y a une cuisine et des W.-C. ? Oui, il y a une cuisine et des W.-C.
            \item Y a-t-il aussi une salle de bains ? Oui, il y en a une.
        \end{enumerate}

\chapter{Leçon 21}
    \section{Notes}
        \begin{itemize}
            \item rendre visite à + person (to visite someone)
            \item faire une surprise à 
            \item avoir le plaisir de + inf (to be happy to do something)
            \item être conjugation doesn't agree in gender when it is indirect object.
            \item d'après (according to)
        \end{itemize}

    \section{E2}
        \begin{enumerate}
            \item Est-ce que vous avez beaucoup d'amis? Non, je n'ai pas beaucoup d'amis.
            \item Vous vous voyez souvent? Non, je ne vous vois pas souvent.
            \item Quand vous allez voir des amis, est-ce que vous leur téléphonez à l'avance? Oui, je lui téléphone.
            \item Aimez-vous bavarder avec vos amis? Oui, j'aime bavarder avec leurs.
            \item Avez-vous fait des voyages ensemble? Oui, je fais des voyages ensembles.
            \item Où étes-vous allés? Nous allons à Toronto.
        \end{enumerate}
        Correction:
        \begin{enumerate}
            \item Non, nous ne nous voyons pas souvent.
            \item Oui, je leur téléphone à l'avance.
            \item Oui, j'aime bavarder avec eux.
            \item Oui, nous avons fait des voyages ensemble.
            \item Nous sommes allés à Toronto.
        \end{enumerate}

\chapter{Leçon 22}
    \section{Note}
        \begin{itemize}
            \item soit A soit B (either \dots or \dots)
        \end{itemize}
    \section{E2}
        \begin{enumerate}
            \item Est-ce que vous téléphonez souvent? Non, je ne téléphone pas souvent.
            \item À qui téléphonez-vous souvent? Je téléphone mes amis.
            \item Est-ce qu'il y a un appareil téléphonique chez vous? Non, je ne l'ai pas.
            \item Est-ce que la ligne est souvent occupée? Non, pas de tout.
            \item Quand on téléphone dans une cabine publique, est-ce qu'il faut payer la communication? Oui, il faut payer la communication.
            \item Est-ce qu'il y a des cabines téléphoniques dans la rue? Oui, il y les a dans la rue.
        \end{enumerate}
        Correction:
        \begin{enumerate}
            \item À qui téléphonez-vous souvent ? Je téléphone souvent à mes amis.
            \item Est-ce qu'il y a un appareil téléphonique chez vous ? Non, il n'y en a pas.
            \item Est-ce que la ligne est souvent occupée ? Non, pas du tout.
            \item Est-ce qu'il y a des cabines téléphoniques dans la rue ? Oui, il y en a dans la rue.
        \end{enumerate}

\chapter{Leçon 23}
    \section{Note}
        \begin{itemize}
            \item à la place de (replace \dots)
            \item passer (se mettre) à table
            \item être à table 
            \item préfère \dots que de \dots (rather \dots than \dots)
            \item avoir le temps de + inf (to have time to do something)
        \end{itemize}

    \section{E2}
        \begin{itemize}
            \item Qu'est-ce que vous mangez le matin? Je mange un pain.
            \item Prenez-vous du lait le matin? Oui, j'en prends.
            \item À midi, est-ce que vous déjeunez chez vous? Non, je ne mange pas le déjeuner.
            \item Est-ce que les plats sont bons à la cantine? Non, ils goûtient très mauvais.
            \item Vous buvez souvent du vin? Non, pas du tout.
            \item Est-ce que vous savez faire la cuisine? Oui, je cuisine souvent.
            \item Le dimanche, est-ce que vous invitez souvent des amis à venir déjeuner chez vous? Non, rarement.
            \item Savez-vous la différence entre la cuisine chinoise et la cuisine occidentale? Il y a trop différence.
        \end{itemize}
        Correction:
        \begin{itemize}
            \item Qu'est-ce que vous mangez le matin ? Je mange du pain.
            \item À midi, est-ce que vous déjeunez chez vous ? Non, je ne déjeune pas chez moi.
            \item Est-ce que les plats sont bons à la cantine ? Non, ils ont très mauvais goût.
            \item Est-ce que vous savez faire la cuisine ? Oui, je sais faire la cuisine. Je cuisine souvent.
            \item Savez-vous la différence entre la cuisine chinoise et la cuisine occidentale ? Oui, il y a beaucoup de différences.
        \end{itemize}

\chapter{Leçon 24}
    \section{Note}
        \begin{itemize}
            \item se mettre à + inf (begin to do \dots)
            \item faire signe à qn
            \item passer avant (more important, in front)
            \item mettre + time / money (cost)
            \item se mettre à (start)
            \item accepter de + inf (agree)
            \item au lieu de (rather than)
            \item puisque (introduce the reason in compond sentence)
            \item il suffit de \dots pour \dots (only need \dots to \dots)
        \end{itemize}

    \section{E2}
        \begin{enumerate}
            \item Aimez-vous aller vous promener? Non, je préfère faire la course.
            \item Quand vous promenez-vous? Je promene tout les jours.
            \item Avec qui vous promenez-vous? Je promene seul.
            \item Où vous promenez-vous? Autour mon quartier.
            \item Aimez-vous faire des excursions avec vos camarades? Oui, beaucoup.
        \end{enumerate}
        Correction:
        \section{E2}
        \begin{enumerate}
            \item Aimez-vous aller vous promener ? Non, je préfère courir.
            \item Quand vous promenez-vous ? Je me promène tous les jours.
            \item Avec qui vous promenez-vous ? Je me promène seul.
            \item Où vous promenez-vous ? Je me promène dans mon quartier.
        \end{enumerate}

\chapter{Leçon 25}
    \section{Note}
        \begin{itemize}
            \item La plupart de (the noun following is usually plural and use definitive article)1
        \end{itemize}

    \section{E2}
        \begin{enumerate}
            \item Est-ce que vous faites souvent les courses? Non, je ne fais jamais les courses.
            \item Où achetez-vous des légumes? J'achète des légumes dans le marche.2
        \end{enumerate}
        Correction:
        \begin{enumerate}
            \item Où achetez-vous des légumes ? J'achète des légumes au marché.
        \end{enumerate}

\chapter{Leçon 26}
    \section{Notes}
        \begin{enumerate}
            \item s'étendre sur (area), la superficie
            \item s'étendre (expand)
            \item être baigné par
            \item à la fois (at the same time)
            \item s'occuper de (take care of)
        \end{enumerate}

    \section{E2}
        \begin{enumerate}
            \item Où se situe la Chine? La Chine se situe entre 40 et 80 de latitude Nord.
            \item Quellle est sa population? Elle a popluation de 14 000 000 00 peuple envrion.
            \item La Chine erst-elle un pays en développement? Oui, elle est d'une pays en développement.
            \item Quels sont les fleuves principaux de la Chine? Les fleuves principaux comprend:
            \item Est-ce que Beijing est la ville la plus importante de la Chine? Oui, c'est la plus importante ville de la Chine.
            \item Quelle est la superficie de la Chine? La Chine s'étend sur 960 000 kilomètres carrés environ.
            \item Est-elle plus grande que les États-Unis? Non, elle est aussi grande que les États-Unis environ.
        \end{enumerate}
        Correction:
        \begin{enumerate}
            \item Où se situe la Chine ? La Chine se situe en Asie de l'Est.
            \item Quelle est sa population ? Elle a une population d'environ 1,4 milliard d'habitants.
            \item La Chine est-elle un pays en développement ? Oui, c'est un pays en développement.
            \item Quels sont les principaux fleuves de la Chine ? Les principaux fleuves comprennent le Yangtsé et le fleuve Jaune.
            \item Est-ce que Beijing est la ville la plus importante de la Chine ? Oui, c'est la ville la plus importante de la Chine.
            \item Quelle est la superficie de la Chine ? La superficie de la Chine est d'environ 9 600 000 kilomètres carrés.
            \item Est-elle plus grande que les États-Unis ? Non, elle est à peu près aussi grande que les États-Unis.
        \end{enumerate}

\chapter{Leçon 27}
    \section{Note}
        \begin{itemize}
            \item en plein + nom (in the middle of)
            \item le souvenir (memory, souvenir)
            \item faire les quais (stride along the river)
            \item s'amuser à + inf (do \dots as leisure)
        \end{itemize}

    \section{E2}
        \begin{enumerate}
            \item Habitez-vous dans quelle ville? J'habite à Toronto.
            \item C'est le Queensway qui est le quartier le plus fréquenté de Toronto.
            \item On se trouve beaucoup de magasins dans ce quartier.
            \item Y a-ti-il des monuments historiques dans votre ville? Oui, il y a un peu des monuments historiques.
            \item Est-ce qu'il y a souvent des touristes étrangers dans votre ville? Non, pas du tout.
            \item Est-ce que vous aimez votre ville? Pourquoi? Non, la circulation est trop mal.
        \end{enumerate}
        Correction:
        \begin{enumerate}
            \item Quel est le quartier le plus fréquenté de Toronto ? Le quartier le plus fréquenté de Toronto est The Queensway.
            \item On trouve beaucoup de magasins dans ce quartier.
            \item Y a-t-il des monuments historiques dans votre ville ? Oui, il y a quelques monuments historiques.
            \item Est-ce que vous aimez votre ville ? Pourquoi ? Non, parce que la circulation est très mauvaise.
        \end{enumerate}

\section{Leçon 28}
    \section{Note}
        \begin{itemize}
            \item tirer qn (qch) de \dots (to get out of \dots)
            \item sur le plan + adj. (from \dots side / POV)
            \item être à l'ordre du jour (everyone pays attention to)
            \item avoir lieu (take place)
            \item avoir l'occasion de + inf (have a chance to do something)
            \item opposer qch (qn) à qch (qn) (oppose to)
            \item s'opposer à (prevent, disagree)
            \item volontiers (agree / usually)
            \item changer qch (change)
            \item changer de (modify)
        \end{itemize}

    \section{E2}
        \begin{enumerate}
            \item Il y a vingt-six provinces en Chine.
            \item Êtes-vous de quelle province? Je suis de Beijing.
            \item Qu'est-ce qu'on cultive dans votre province? On se trouve la pastèque.
            \item Est-ce que votre province est développée sur le plan économique et culturel? Oui, exactement.
            \item Quelles sont les villes principales dans votre province? Il n'y a qu'un seul.
        \end{enumerate}
        Correction:
        \begin{enumerate}
            \item Il y a vingt-trois provinces en Chine.
            \item Qu'est-ce qu'on cultive dans votre région ? On y cultive des pastèques.
            \item Est-ce que votre région est développée sur les plans économique et culturel ? Oui, elle est très développée.
            \item Quelles sont les villes principales de votre région ? Il n'y en a qu'une seule.
        \end{enumerate}

\chapter{Leçon 29}
    \section{Note}
        \begin{itemize}
            \item to express program, use ``en''
            \item qch. plaire à qn (something made someone interesting)
            \item c'est-à-dire (used for explanation or clarification)
            \item visiter qn (used for visiting ill people), use rendre visite à qn or aller voir qn.
            \item c'est \dots qui (to emphasize pronoun)
            \item décider de qch (determine something or determining factor)
            \item être malade de (ill)
            \item prendre le pouls (tâter le pouls)
            \item être touché par (to be touched)
            \item si \dots que (so \dots that)
            \item intéresser qn (make \dots interested)
            \item s'intéresser à (interested in \dots)
            \item guérir qn (d'une maladie), coure someone's illness
            \item être guéri (healed)
            \item décider qch (decide)
            \item décider de + inf (decide to do \dots)
            \item se décider (determined)
        \end{itemize}

\chapter{Leçon 30}
    \begin{itemize}
        \item Ça fait + time + que (it has been)
        \item Qu'est-ce qui / qui est-ce qui (to question the pronoun, ``que'' cannot be pronoun, so we need qu'est-ce qui)
        \item ce qui \dots c'est \dots (to emphasize)
        \item au bout de (if used for time, means after \dots, for space, means at the end)
        \item être persuadé que (to be convinced)
        \item autres (used after nous, vous, eux, means people like us)
        \item toutes sortes de (various)
        \item abonder en qch (abundance in)
        \item penser qch de qn / qch (have thoughts on \dots)
        \item lutter contre (fight against)
        \item se faire sentir (start to feel)
        \item disinguer qch (qn) de qch (qn), distinguish bewteen
        \item s'indigner de
        \item continuer à / de
    \end{itemize}
